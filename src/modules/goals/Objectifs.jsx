...
import {
    ...
    MagnifyingGlassIcon, XMarkIcon, AdjustmentsHorizontalIcon, ChevronLeftIcon, ChevronRightIcon
...
const progressColor = (p) => p >= 100 ? '#14b8a6' : p >= 50 ? '#8b5cf6' : '#f59e0b';
const progressBg    = (p) => p >= 100 ? 'bg-teal-400' : p >= 50 ? 'bg-violet-400' : 'bg-amber-300';
const progressLabel = (p) => p >= 100
    ? { bg: 'bg-teal-50 text-teal-600 border-teal-200',     label: 'Atteint'  }
    : p >= 50
    ? { bg: 'bg-violet-50 text-violet-600 border-violet-200', label: 'En cours' }
    :   { bg: 'bg-amber-50 text-amber-600 border-amber-200',  label: 'À risque' };
...
const ObjectifCard = ({ goal, onEdit, onClose, isAdmin }) => {
    ...
    return (
        <motion.div
            ...
            className="group bg-white rounded-2xl border border-slate-200/70 shadow-sm hover:shadow-md hover:border-slate-300 transition-all duration-200 p-5 flex flex-col relative overflow-hidden"
        >
            <div className={`absolute top-0 left-0 right-0 h-1 rounded-t-2xl ${progressBg(getProgress(goal))}`} />
            {/* top */}
            <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                        <h4 className="text-sm font-bold text-slate-800 leading-tight truncate max-w-[140px]">{goal.TypeObjectif}</h4>
                        {goal.utilisateur && (
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-none">
                    {onEdit && (
                        <button onClick={onEdit}
                            className="h-7 w-7 flex items-center justify-center rounded-lg bg-slate-100 text-slate-400 hover:bg-slate-200 hover:text-slate-600 transition-colors">
                            <EyeIcon className="h-3.5 w-3.5" />
                        </button>
                    )}
                    {isAdmin && onClose && (
                        <button onClick={onClose}
                            className="h-7 w-7 flex items-center justify-center rounded-lg bg-rose-50 text-rose-400 hover:bg-rose-100 transition-colors">
            ...
            <div className="flex items-center gap-4 mb-4">
                <CircleProgress progress={progress} size={56} />
                <div className="flex-1 min-w-0">
                    <div className="flex items-baseline gap-1">
                        <span className="text-lg font-black text-slate-800 leading-none">
                            {formatValue(getObjectifRealised(goal), isCount)}
                        </span>
                    </div>
                    <p className="text-xs text-slate-400 mt-0.5">
                        sur {formatValue(getObjectifTarget(goal), isCount)} {unit}
                    </p>
                    {(goal.DateDebut || goal.DateFin) && (
                        <p className="text-[10px] text-slate-400 mt-1.5 flex items-center gap-1">
                            <CalendarIcon className="h-3 w-3 flex-none" />
...
const Objectifs = () => {
    ...
    const kpis = [
        {
            label: 'Atteinte globale', value: `${Math.round(globalProgress)}%`, sub: 'progression',
            icon: ArrowTrendingUpIcon, trend: globalProgress >= 50 ? 'up' : 'down',
            from: 'from-violet-300', to: 'to-purple-300', bg: 'bg-violet-50', text: 'text-violet-500'
        },
        {
            label: 'Objectif total', value: totalTarget >= 1000 ? `${(totalTarget/1000).toFixed(1)}k` : Math.round(totalTarget).toLocaleString(), sub: 'TND / période',
            icon: FlagIcon,
            from: 'from-rose-300', to: 'to-pink-300', bg: 'bg-rose-50', text: 'text-rose-500'
        },
        {
            label: 'Réalisé', value: totalRealised >= 1000 ? `${(totalRealised/1000).toFixed(1)}k` : Math.round(totalRealised).toLocaleString(), sub: 'TND cumulés',
            icon: ChartBarIcon, trend: 'up',
            from: 'from-teal-300', to: 'to-emerald-300', bg: 'bg-teal-50', text: 'text-teal-600'
        },
        {
            label: 'Écart restant', value: (totalTarget-totalRealised) >= 1000 ? `${((totalTarget-totalRealised)/1000).toFixed(1)}k` : Math.round(Math.max(0, totalTarget-totalRealised)).toLocaleString(), sub: 'TND à faire',
            icon: ArrowPathIcon,
            from: 'from-amber-300', to: 'to-orange-300', bg: 'bg-amber-50', text: 'text-amber-500'
        },
    ];
    ...
    return (
        <div className="animate-fade-in space-y-6 pb-12">
            ...
            <div className="rounded-2xl relative overflow-hidden bg-white border border-slate-200/80 shadow-sm p-7 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                ...
                <div className="absolute inset-0 bg-gradient-to-br from-violet-50/40 via-white to-indigo-50/30 pointer-events-none" />
                <div className="absolute top-0 right-0 w-64 h-full bg-gradient-to-l from-violet-50/50 to-transparent pointer-events-none" />

                <div className="relative">
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-violet-50 border border-violet-200 text-violet-600 text-xs font-semibold mb-3">
                        <SparklesIcon className="h-3.5 w-3.5" /> Performances
                    </span>
                    ...
                    <div className="flex items-center gap-2.5 mt-5 flex-wrap">
                        {isAdmin && (
                            <button onClick={() => setShowArchived(v => !v)}
                                className="inline-flex items-center gap-2 h-9 px-4 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-600 text-sm font-medium border border-slate-200 transition-colors">
                        ...
                        {canCreate && (
                            <button onClick={() => navToNew()}
                                className="inline-flex items-center gap-2 h-9 px-4 rounded-xl bg-slate-700 hover:bg-slate-800 text-white text-sm font-semibold shadow-sm transition-all active:scale-95">
                ...
                <div className="relative flex items-center gap-5 bg-slate-50 rounded-2xl px-6 py-4 border border-slate-200/80 flex-shrink-0">
                    <div className="relative">
                        <svg width="84" height="84" viewBox="0 0 84 84">
                            <circle cx="42" cy="42" r="32" fill="none" stroke="#e2e8f0" strokeWidth="7" />
                            <circle cx="42" cy="42" r="32" fill="none" stroke="#a78bfa" strokeWidth="7"
                                strokeDasharray={2 * Math.PI * 32}
                                strokeDashoffset={2 * Math.PI * 32 - (Math.min(globalProgress,100)/100) * 2 * Math.PI * 32}
                                strokeLinecap="round" transform="rotate(-90 42 42)"
                                style={{ transition: 'stroke-dashoffset 0.8s ease' }} />
                            <text x="42" y="47" textAnchor="middle" fontSize="14" fontWeight="900" fill="#7c3aed" fontFamily="system-ui">
                    ...
                    <div>
                        ...
                        <p className="text-slate-800 font-bold text-lg leading-tight">{visibleObjectifs.length} objectif{visibleObjectifs.length !== 1 ? 's' : ''}</p>
                        <p className="text-violet-400 text-xs mt-0.5">{selectedYear}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {kpis.map((k, i) => (
                    <div key={i} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex flex-col gap-3 hover:shadow-lg transition-all duration-200 relative overflow-hidden group">
                        <div className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${k.from} ${k.to}`} />
                        <div className="flex items-center justify-between">
                            <div className={`h-10 w-10 rounded-xl ${k.bg} flex items-center justify-center`}>
                                <k.icon className={`h-5 w-5 ${k.text}`} />
                            </div>
                            {k.trend && (
                                <span className={`flex items-center gap-0.5 text-xs font-semibold px-2 py-1 rounded-lg ${k.trend === 'up' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
                ...
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                        ...
                        {(filterType !== 'all' || filterStatus !== 'all' || searchTerm) && (
                            <span className="h-5 min-w-[1.25rem] px-1 rounded-full bg-slate-600 text-white text-[10px] font-bold flex items-center justify-center">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-1 bg-slate-100 rounded-xl px-1 py-1">
                            <button onClick={() => setSelectedYear(y => y - 1)} className="h-7 w-7 flex items-center justify-center rounded-lg hover:bg-white text-slate-500 transition-colors">
                ...
                <div className="flex flex-wrap gap-3">
                    <div className="relative flex-1 min-w-[180px]">
                        ...
                        <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                            ...
                            className="w-full pl-9 pr-4 h-9 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-300 focus:border-slate-300" />
                    </div>
                    {isAdmin && (
                        <select value={selectedUserId} onChange={e => setSelectedUserId(e.target.value)}
                            className="h-9 px-3 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-300 text-slate-700">
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                    <div className="flex items-center justify-between mb-5">
                        ...
                        <span className="flex items-center gap-1.5 text-xs text-slate-400">
                            <span className="h-2 w-2 rounded-full bg-violet-500" /> Réalisé
                        </span>
                    </div>
                    {chartData.length === 0 ? (
                        <div className="h-52 flex items-center justify-center text-sm text-slate-400 italic">Aucune donnée pour cette période</div>
                    ) : (
                        <div className="h-52">
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={chartData} margin={{ top:5, right:5, left:0, bottom:0 }}>
                                    ...
                                    <Area type="monotone" dataKey="sales" stroke="#7c3aed" strokeWidth={2.5} fill="url(#gradCA)" dot={{ r:3, fill:'#7c3aed' }} />
                                </AreaChart>
                            </ResponsiveContainer>
                        </div>
                    )}
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-violet-50/60 to-white">
                    <div className="flex items-center gap-3">
                    ...
                    {canCreate && (
                        <button onClick={() => navToNew({ typePeriode: 'Mensuel' })}
                            className="inline-flex items-center gap-1.5 h-8 px-4 rounded-xl bg-slate-700 text-white text-xs font-semibold hover:bg-slate-800 transition-colors shadow-sm">
                    ...
                    {reduxLoading && monthlyObjectifs.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                            {monthlyObjectifs.map(goal => (
                                <ObjectifCard key={goal.ID_Objectif} goal={goal} isAdmin={isAdmin}
                                    onEdit={() => navigate(`/objectifs/edit/${goal.ID_Objectif}`, { state: { objectif: goal } })}
                                    onClose={() => handleAdminCloseObjectif(goal)} />
                            ))}
                            {canCreate && (
                                <button onClick={() => navToNew({ typePeriode: 'Mensuel' })}
                                    className="flex flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/50 p-6 text-center hover:border-violet-300 hover:bg-violet-50/30 transition-all group min-h-[160px]">
                                    <div className="h-12 w-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center group-hover:border-violet-300 group-hover:text-violet-500 text-slate-300 transition-all">
                                        <PlusIcon className="h-6 w-6 stroke-[2.5]" />
                                    </div>
                                    <p className="text-xs font-semibold text-slate-400 group-hover:text-violet-600 transition-colors">Nouvel objectif</p>
                                </button>
                            )}
                        </div>
                    ) : (
                        <div className="py-16 flex flex-col items-center text-center">
                            <div className="h-14 w-14 rounded-2xl bg-slate-100 flex items-center justify-center mb-4">
                                <FlagIcon className="h-7 w-7 text-slate-300" />
                            </div>
                            ...
                            <button onClick={() => { setFilterType('all'); setFilterStatus('all'); setSearchTerm(''); }}
                                className="h-8 px-4 rounded-xl border border-slate-200 text-xs font-semibold text-slate-600 bg-white hover:bg-slate-50">
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-emerald-50/60 to-white">
                    <div className="flex items-center gap-3">
                                ...
                                {archivedObjectifs.map(goal => {
                                    ...
                                    return (
                                        <motion.div key={`arch-${goal.ID_Objectif}`}
                                            ...
                                            className="bg-slate-50 rounded-2xl border border-slate-200 p-5 flex flex-col opacity-70 hover:opacity-100 transition-opacity">
            ...
            {isAdmin && commercialRanking.length > 0 && (
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                    <div className="flex items-center gap-3 px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-amber-50/60 to-white">
                        ...
                        <table className="w-full">
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {commercialRanking.map((item, i) => {
                                    ...
                                    return (
                                        <tr key={item.userId} className={`hover:bg-slate-50 transition-colors ${isCurrent ? 'bg-violet-50/40' : ''}`}>
