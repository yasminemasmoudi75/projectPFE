...
import { updateBlv, createBlv, fetchBlvById, clearCurrentBlv } from './blvSlice';
...
const BlvForm = () => {
    ...
    // Cin is stored separately - it's a Tiers field, not part of BlvMaster
    ...
    useEffect(() => {
        if (isEdit && currentBlv) {
            const { details, ...master } = currentBlv;
            setFormData({
                ...master,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEdit) {
                await dispatch(updateBlv({ id, payload })).unwrap();
                toast.success('Blv mis à jour');
            } else {
                await dispatch(createBlv(payload)).unwrap();
                toast.success('Blv créé avec succès');
            }
            navigate('/blv');
        } catch (err) {
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight">
                            {isEdit ? `Modification BL N°${formData.Nf}` : 'Nouveau Bon de Livraison'}
                        </h1>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    >
                        ...
                        {isEdit ? 'Mettre à jour' : 'Finaliser la Livraison'}
                    </button>
                </div>
            </div>

            <form id="Blv-form" onSubmit={handleSubmit} className="max-w-4xl mx-auto space-y-8 font-sans">
