...
const FavDetail = () => {
  ...
  const handleTransfer = async (targetType) => {
    try {
      const response = await api.post(`/fav/${id}/transfer`, { targetType });
      if (response?.status === 'success') {
        toast.success(`Transféré avec succès vers Bon de Livraison`);
      ...
      console.error('Erreur transfert FAV:', err);
