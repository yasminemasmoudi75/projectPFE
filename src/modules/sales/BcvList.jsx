...
const BcvList = () => {
  ...
  // Apply filters to Bcv list
  const filteredBcv = bcv?.filter(item => {
    // Search filter
    ...
    if (filters.status !== 'all') {
      ...
      if (filters.status === 'valid' && (!item.Valid || item.bLivr)) return false;
      if (filters.status === 'converted' && !item.bLivr) return false;
    }
  ...
  const fetchCommercials = async () => {
    try {
      const params = {
        moduleCode: String(MODULE_CODES.COMMANDES),
  ...
  // Export functions
  const exportToCSV = () => {
    const headers = ['N° Commande', 'Date', 'Client', 'Montant HT', 'Montant TTC', 'Statut Livraison'];
    const rows = filteredBcv.map(b => [
      `${b.Prfx || ''}${b.Nf || ''}`,
      b.DatUser ? new Date(b.DatUser).toLocaleDateString('fr-FR') : '',
      b.LibTiers || '',
      b.TotHT || 0,
      b.TotTTC || 0,
      b.bLivr ? 'Livré' : 'En attente'
    ]);
      ...
      <html>
        ...
        <body>
          <div class="header">
            ...
            <p>Exporté le ${new Date().toLocaleDateString('fr-FR')} - ${filteredBcv.length} commandes</p>
          </div>
          <table>
            <thead>
              <tr>
            ...
            <tbody>
              ${filteredBcv.map(b => {
                const statusClass = b.bLivr ? 'badge-delivered' : 'badge-pending';
                const statusText = b.bLivr ? 'Livré' : 'En attente';
                return `
                  <tr>
                    <td>${b.Prfx || ''}${b.Nf || ''}</td>
  ...
  useEffect(() => {
    if (!isFilterRepresEnabled) {
      setAllClients(clientsFromVisibleBcv);
    }
  ...
  const totalBcvTTC = filteredBcv?.reduce((acc, curr) => acc + (curr.TotTTC || 0), 0) || 0;
  const deliveredCount = filteredBcv?.filter(item => item.bLivr).length || 0;
  const deliveryRate = filteredBcv?.length > 0 ? ((deliveredCount / filteredBcv.length) * 100).toFixed(1) : 0;

  // Calculate Pagination
  const totalItems = filteredBcv.length;
  const totalPages = Math.ceil(totalItems / itemsPerPage) || 1;
  const startIndex = (currentPage - 1) * itemsPerPage;
  const paginatedBcv = filteredBcv.slice(startIndex, startIndex + itemsPerPage);
          ...
          <div className="p-6 flex items-center justify-between relative z-10">
            <div>
              ...
              <p className="text-3xl font-extrabold text-slate-900 bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-indigo-600">
                {totalBcvTTC > 0 ? (totalBcvTTC / 1000).toFixed(1) + 'k' : '0'} <span className="text-sm text-slate-400 font-bold ml-1">TND</span>
              </p>
            </div>
            ...
            <div>
              ...
              <div className="flex items-end gap-2">
                ...
                <span className="mb-1 text-xs px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 font-bold flex items-center gap-1">
                  <ArrowTrendingUpIcon className="h-3 w-3" /> Taux Livraison
                </span>
              </div>
            </div>
        ...
        <div className="p-4 sm:p-6 flex flex-col xl:flex-row gap-4 xl:items-center">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {[
              ...
              { id: 'converted', label: 'Livrés', color: 'emerald' }
            ].map(status => {
        ...
        <div className="overflow-x-auto bg-slate-50/30">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100/80 bg-white">
              <AnimatePresence>
                {(!filteredBcv || filteredBcv.length === 0) ? (
                  <motion.tr
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                      ...
                      >
                        ...
                        <div>
                          <p className="text-slate-800 font-bold mb-1">
                            {bcv && bcv.length > 0 ? 'Aucun résultat trouvé' : 'Dossier vide'}
                          </p>
                        ...
                        {(!bcv || bcv.length === 0) && canCreate && (
                          <button onClick={() => navigate('/bcv/new')} className="mt-2 text-sm text-blue-600 font-bold hover:text-blue-700">
                            + Créer un Bcv
                          </button>
                        )}
                      </motion.div>
                    ...
                    >
                      ...
                      <td className="px-8 py-4">
                        <span className={clsx(
                          ...
                          {item.bLivr ? "Livré" : item.Valid ? "Validé" : "Brouillon"}
                        </span>
                      </td>
                      <td className="px-8 py-4">
