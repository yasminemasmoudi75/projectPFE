...
const STATUS_CONFIG = {
  ...
  converted: { label: 'Livré', bar: 'bg-emerald-500', badge: 'bg-emerald-50 text-emerald-700 border-emerald-200', dot: 'bg-emerald-500' },
};

const getStatus = (item) => {
  if (item.bLivr || item.IsConverted) return 'converted';
...
const BcvList = () => {
  ...
  const clientsFromVisibleBcv = useMemo(() => {
    const map = new Map();
    (bcv || []).forEach((item) => {
  ...
  // ── Filtered list ──────────────────────────────────────────────────────────
  const filteredBcv = useMemo(() => (bcv || []).filter((item) => {
    if (filters.search) {
      const q = filters.search.toLowerCase();
      if (!(`${item.Prfx}${item.Nf}`.toLowerCase().includes(q) || (item.LibTiers || '').toLowerCase().includes(q))) return false;
    }
    if (filters.status !== 'all') {
      if (filters.status === 'draft' && (item.Valid || item.bLivr || item.IsConverted)) return false;
      if (filters.status === 'valid' && (!item.Valid || item.bLivr || item.IsConverted)) return false;
      if (filters.status === 'converted' && !(item.bLivr || item.IsConverted)) return false;
    }
  ...
  const convertedCount = filteredBcv.filter((i) => i.bLivr || i.IsConverted).length;
  const conversionRate = filteredBcv.length > 0 ? ((convertedCount / filteredBcv.length) * 100).toFixed(1) : '0';
  const avgAmount = filteredBcv.length > 0 ? totalCA / filteredBcv.length : 0;

  // ── Pagination ─────────────────────────────────────────────────────────────
  const totalItems = filteredBcv.length;
  const totalPages = Math.max(1, Math.ceil(totalItems / ITEMS_PER_PAGE));
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const paginatedBcv = filteredBcv.slice(startIndex, startIndex + ITEMS_PER_PAGE);
  ...
  const fetchCommercials = async () => {
    try {
      const res = await axios.get('/users/commercials/devis-filter', {
        params: { moduleCode: String(MODULE_CODES.COMMANDES), includeAll: isFilterRepresEnabled ? 'false' : 'true' },
      });
  ...
  const STATUS_BTNS = [
    ...
    { id: 'converted', label: 'Livrées', active: 'bg-emerald-50 text-emerald-700 border-emerald-300' },
  ];
  ...
  return (
    <motion.div variants={pageVariants} initial="hidden" animate="visible" className="space-y-6 pb-12">
      ...
      <motion.div variants={fadeUp} className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Carnet de Commandes</h1>
            <span className="relative flex h-2.5 w-2.5 mt-0.5">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-60" />
              <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-blue-500" />
            </span>
          </div>
          <p className="text-sm text-slate-500">Gérez vos commandes clients et suivez vos ventes.</p>
        </div>
      ...
      <motion.div variants={fadeUp} className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard
          label="Montant Commandé" sub="TND — filtré"
        ...
        />
        <KpiCard
          label="Commandes reçues" sub="documents trouvés"
          value={filteredBcv.length}
          icon={ShoppingCartIcon} accent="bg-emerald-500"
          iconBg="bg-emerald-50" valueColor="text-emerald-700"
        />
        <KpiCard
          label="Taux de livraison" sub="commandes livrées"
          value={`${conversionRate}%`}
          icon={ArrowTrendingUpIcon} accent="bg-amber-400"
          iconBg="bg-amber-50" valueColor="text-amber-700"
        />
        <KpiCard
          label="Commande moyenne" sub="TND par commande"
                      ...
                      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center gap-4">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-slate-700 mb-1">
                            {bcv?.length > 0 ? 'Aucun résultat' : 'Dossier vide'}
                          </p>
                          <p className="text-xs text-slate-400">
                            {bcv?.length > 0 ? 'Ajustez vos filtres pour trouver ce que vous cherchez.' : "Aucune commande créée pour le moment."}
                          </p>
                        </div>
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex justify-end items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                            {canShowEditAction && (
                              <button
                                onClick={(e) => { e.stopPropagation(); navigate(`/bcv/edit/${item.Guid}`); }}
                                title="Modifier"
