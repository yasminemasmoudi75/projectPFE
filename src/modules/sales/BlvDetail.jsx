...
const BlvDetail = () => {
  ...
  const handleDownloadPDF = async () => {
    try {
      const response = await api.get(`/blv/${id}/pdf`, { responseType: 'blob' });
  ...
  if (!blv) return (
    <div className="flex flex-col items-center justify-center py-32 gap-4">
      ...
      <button onClick={() => navigate('/blv')} className="text-xs text-slate-500 hover:text-slate-700">← Retour à la liste</button>
    </div>
  );

  const ref     = `${blv.Prfx || 'BL'}${blv.Nf}`;
  const totalHT  = Number(blv.TotHT  || 0);
  const totalTVA = Number(blv.TotTva || 0);
  const totalRem = Number(blv.TotRem || 0);
  const totalTTC = Number(blv.TotTTC || 0);
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden print:hidden">
        <div className="px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-5">
          ...
          <div className="flex items-center gap-4 min-w-0">
            ...
            <div className="h-11 w-11 rounded-xl bg-gradient-to-br from-teal-500 to-sky-500 flex items-center justify-center flex-shrink-0 shadow-md shadow-teal-500/20">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 text-xs text-slate-400 flex-wrap">
                {blv.LibTiers && <span className="font-semibold text-slate-600 truncate max-w-[200px]">{blv.LibTiers}</span>}
                {blv.LibTiers && blv.DatUser && <span className="text-slate-300">·</span>}
                {blv.DatUser && (
                  <span className="flex items-center gap-1">
                    <CalendarDaysIcon className="h-3 w-3 text-slate-400" />
                    {formatDate(blv.DatUser)}
                  </span>
                )}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden print:shadow-none print:border-none">
        <div className="px-12 py-10 print:p-8">
          ...
          <div className="text-center mb-10">
            <h2 className="text-[22px] font-black text-slate-900 uppercase tracking-[0.3em]">Bon de Livraison</h2>
          ...
          <div className="grid grid-cols-2 gap-12 mb-10 pb-10 border-b border-slate-100">
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-slate-700">{formatDate(blv.DatUser) || '—'}</span>
              </div>
              {transport?.nom && (
                <>
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-500">Chauffeur</span>
                    <span className="font-semibold text-slate-700">{transport.nom}</span>
                  </div>
                  {transport.tel && (
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-500">Tél. transport</span>
                      <span className="font-semibold text-slate-700">{transport.tel}</span>
                    </div>
                  )}
                </>
              )}
              {blv.Remarq && !blv.Remarq.includes('__transport__') && (
                <div className="pt-3 border-t border-slate-100">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Remarques</p>
                  <p className="text-xs text-slate-500 italic leading-relaxed">{blv.Remarq}</p>
                </div>
              )}
            </div>
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Destinataire</p>
              <h3 className="text-xl font-black text-slate-900 leading-tight mb-2">{blv.LibTiers}</h3>
              <div className="text-sm text-slate-500 space-y-0.5 leading-relaxed">
                {blv.Adresse && <p>{blv.Adresse}</p>}
                {blv.Ville && <p className="font-semibold text-slate-700">{blv.Ville}</p>}
                {blv.Cin && <p className="font-mono text-slate-400 text-xs">ID : {blv.Cin}</p>}
              </div>
            </div>
          </div>
            ...
            <div className="rounded-xl border border-slate-200 overflow-hidden">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100">
                  ...
                  {(!blv.details || blv.details.length === 0) && (
            ...
            <div className="w-full sm:w-80 space-y-2">
              <div className="space-y-2 pb-4 border-b border-slate-200">
                {[
                  ...
                  { label: `TVA (${blv.details?.[0]?.Tva ?? 19}%)`, value: totalTVA, color: 'text-slate-600' },
