...
const DevisList = () => {
  ...
  const convertedCount = filteredDevis.filter((i) => i.IsConverted).length;
  const conversionRate = filteredDevis.length > 0 ? ((convertedCount / filteredDevis.length) * 100).toFixed(1) : '0';
  const avgAmount = filteredDevis.length > 0 ? totalCA / filteredDevis.length : 0;
...
const refreshData = () => {
  ...
  const fetchCommercials = async () => {
    try {
      const res = await axios.get('/users/commercials/devis-filter', {
        params: { moduleCode: String(MODULE_CODES.DEVIS), includeAll: isFilterRepresEnabled ? 'false' : 'true' },
      });
  ...
  useEffect(() => {
    if (!isFilterRepresEnabled) setAllClients(clientsFromVisibleDevis);
  ...
  const STATUS_BTNS = [
    ...
    { id: 'converted', label: 'Transformés', active: 'bg-emerald-50 text-emerald-700 border-emerald-300' },
  ];
  ...
  return (
    <motion.div variants={pageVariants} initial="hidden" animate="visible" className="space-y-6 pb-12">
      ...
      <motion.div variants={fadeUp} className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Propositions &amp; Devis</h1>
            <span className="relative flex h-2.5 w-2.5 mt-0.5">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-60" />
              <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-blue-500" />
            </span>
          </div>
          <p className="text-sm text-slate-500">Gérez vos opportunités commerciales et suivez vos propositions.</p>
        </div>
      ...
      <motion.div variants={fadeUp} className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard
          label="Montant Proposé" sub="TND — filtré"
                      ...
                      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center gap-4">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-slate-700 mb-1">
                            {devis?.length > 0 ? 'Aucun résultat' : 'Dossier vide'}
                          </p>
                          <p className="text-xs text-slate-400">
                            {devis?.length > 0 ? 'Ajustez vos filtres pour trouver ce que vous cherchez.' : "Aucun devis créé pour le moment."}
                          </p>
                        </div>
                        {(!devis || devis.length === 0) && canCreate && (
                          <button onClick={() => navigate('/devis/new')} className="mt-1 text-sm text-blue-600 font-semibold hover:text-blue-700 transition-colors">
                            + Créer un devis
                          </button>
                        )}
                      </motion.div>
