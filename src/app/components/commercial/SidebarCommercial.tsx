...
import {
  ...
  Calendar, FileText, Target, LogOut, ChevronDown,
} from 'lucide-react';
import { cn } from '@/app/components/ui/utils';

interface SidebarCommercialProps {
  activeTab: string;
...
const groups = [
  ...
  },
  {
    name: 'Commercial',
    items: [
      { id: 'projets',   label: 'Mes Projets',       icon: Briefcase, color: 'violet' },
  ...
  },
  {
    name: 'Pilotage',
    items: [
      { id: 'objectifs',  label: 'Mes Objectifs', icon: Target,    color: 'emerald' },
      { id: 'calendrier', label: 'Calendrier',    icon: Calendar,  color: 'pink'    },
    ],
  },
];

export function SidebarCommercial({ activeTab, onTabChange }: SidebarCommercialProps) {
        ...
        <div className="px-4 py-4 border-b border-slate-200">
          <div className="w-full flex items-center gap-3 p-3 rounded-lg bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-100 hover:border-blue-300 transition-all cursor-pointer group">
            ...
            <div className="flex-1 min-w-0 text-left">
              <p className="text-sm font-semibold text-slate-900 truncate">Mohamed Commercial</p>
