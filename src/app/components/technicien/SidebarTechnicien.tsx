...
import {
  LayoutDashboard, AlertCircle, Wrench,
  History, CheckSquare, LogOut, ChevronDown,
} from 'lucide-react';
import { cn } from '@/app/components/ui/utils';

interface SidebarTechnicienProps {
  activeTab: string;
...
const groups = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'interventions', label: 'Interventions', icon: Wrench,      color: 'amber'  },
      { id: 'taches',        label: 'Mes Tâches',    icon: CheckSquare, color: 'indigo' },
    ],
  },
  {
    name: 'Suivi',
...
export function SidebarTechnicien({ activeTab, onTabChange }: SidebarTechnicienProps) {
        ...
        <div className="px-4 py-4 border-b border-slate-200">
          <div className="w-full flex items-center gap-3 p-3 rounded-lg bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-100 hover:border-blue-300 transition-all cursor-pointer group">
            ...
            <div className="flex-1 min-w-0 text-left">
              <p className="text-sm font-semibold text-slate-900 truncate">Ahmed Technicien</p>
              <p className="text-xs text-slate-500 truncate">Technicien</p>
            </div>
