...
import {
  LayoutDashboard, ShieldCheck, Users, Building2, Package, Boxes,
  Briefcase, Activity, FileText, Wrench, Target,
  Calendar, BarChart3, Sparkles, LogOut, ChevronDown, ClipboardList,
} from 'lucide-react';
...
const palette: Record<string, { pastel: string; text: string; solid: string }> = {
  ...
  indigo:  { pastel: 'bg-indigo-50',  text: 'text-indigo-600',  solid: 'bg-indigo-500'  },
  emerald: { pastel: 'bg-emerald-50', text: 'text-emerald-600', solid: 'bg-emerald-500' },
  amber:   { pastel: 'bg-amber-50',   text: 'text-amber-600',   solid: 'bg-amber-500'   },
  orange:  { pastel: 'bg-orange-50',  text: 'text-orange-600',  solid: 'bg-orange-500'  },
  teal:    { pastel: 'bg-teal-50',    text: 'text-teal-600',    solid: 'bg-teal-500'    },
  rose:    { pastel: 'bg-rose-50',    text: 'text-rose-600',    solid: 'bg-rose-500'    },
  red:     { pastel: 'bg-red-50',     text: 'text-red-600',     solid: 'bg-red-500'     },
  pink:    { pastel: 'bg-pink-50',    text: 'text-pink-600',    solid: 'bg-pink-500'    },
  cyan:    { pastel: 'bg-cyan-50',    text: 'text-cyan-600',    solid: 'bg-cyan-500'    },
  slate:   { pastel: 'bg-slate-100',  text: 'text-slate-600',   solid: 'bg-slate-500'   },
  purple:  { pastel: 'bg-purple-50',  text: 'text-purple-600',  solid: 'bg-purple-500'  },
};
...
const groups = [
  ...
  },
  {
    name: 'Administration',
    items: [
      { id: 'utilisateurs', label: 'Utilisateurs',        icon: Users,          color: 'indigo'  },
      { id: 'societes',     label: 'Sociétés',            icon: Building2,      color: 'violet'  },
      { id: 'produits',     label: 'Produits & Services', icon: Package,        color: 'orange'  },
      { id: 'stocks',       label: 'Stocks',              icon: Boxes,          color: 'amber'   },
      { id: 'roles',        label: 'Rôles & Permissions', icon: ShieldCheck,    color: 'sky'     },
      { id: 'journal',      label: 'Journal Connexions',  icon: ClipboardList,  color: 'slate'   },
    ],
  },
  {
    name: 'CRM & Ventes',
    items: [
      { id: 'projets',   label: 'Projets',          icon: Briefcase, color: 'violet' },
      { id: 'activites', label: 'Activités',         icon: Activity,  color: 'rose'   },
      { id: 'devis',     label: 'Devis & Commandes', icon: FileText,  color: 'sky'    },
      { id: 'sav',       label: 'SAV',               icon: Wrench,    color: 'red'    },
    ],
  },
  {
    name: 'Intelligence',
    items: [
      { id: 'objectifs',    label: 'Objectifs',    icon: Target,   color: 'emerald' },
      { id: 'calendrier',   label: 'Calendrier',   icon: Calendar, color: 'pink'    },
      { id: 'statistiques', label: 'Statistiques', icon: BarChart3, color: 'cyan'   },
      { id: 'ia',           label: 'IA & Relances', icon: Sparkles, color: 'violet' },
    ],
  },
];

export function SidebarAdmin({ activeTab, onTabChange }: SidebarAdminProps) {
  const [collapsed, setCollapsed] = useState<Record<string, boolean>>({});
  const toggle = (name: string) => setCollapsed(prev => ({ ...prev, [name]: !prev[name] }));

  return (
    /* Outer padding container — identical to front-end `p-3 lg:w-72` */
        ...
        <div className="px-4 py-4 border-b border-slate-200">
          <div className="w-full flex items-center gap-3 p-3 rounded-lg bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-100 hover:border-blue-300 transition-all cursor-pointer group">
            ...
            <div className="flex-1 min-w-0 text-left">
              <p className="text-sm font-semibold text-slate-900 truncate">Admin Principal</p>
